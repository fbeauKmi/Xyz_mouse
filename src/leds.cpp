...
void Orbion_Neopixel::config(uint8_t mode, uint8_t color_mode, uint32_t color1, uint32_t color2){
...
void Orbion_Neopixel::display(uint8_t mode, uint8_t color_mode, uint32_t color1, uint32_t color2){
      config(mode,color_mode,color1,color2);
      display();
      return;
}
...
void Orbion_Neopixel::display(void){    
    ...
    if(millis()-_lastframe > 150)
      ...
      uint8_t end = (_mode ? first + ( _mode ==1 ? 1 : n / 2) : n);
      
      if(_color_mode == 2){
          color = mixColorCycle(_color1,_color2, _lastframe << 3 );
      } else if(_color_mode == 3) {
