...
void Orbion_button::setKey(button_action b){
    Keys = b;
}
...
void Orbion_button::action(){
    if(ispressed){
      if(!_triggered){
        _triggered = !_triggered;
        if(Keys.K){ Keyboard.press(Keys.K);}
        if(Keys.M){ Mouse.press(Keys.M);}
      }
    }else{
      if(_triggered){
        _triggered=!_triggered;
        if(Keys.K){ Keyboard.release(Keys.K);}
        if(Keys.M){ Mouse.release(Keys.M);}
      }
    }
}
